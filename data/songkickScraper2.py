...
def getAllGigographies(db, artist_ids):
  ...
  for index, x in enumerate(artist_ids):
    page = 1  
    while True:
      data = requestGigography(x[1], page)
      if not data:
        break
      for e in data:
        saveEvent(db, e)
        event_ids.append(e['id'])
      LOGGER.info("Saved event data for %s (id: %s) with page %s, [%s/%s artists]", x[0], x[1], page,index+1, total)
      page += 1
